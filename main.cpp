...
int HttpGet(int rL)
{
	std::string url = download_url(r_lua_tolstring(rL, -1, 0));
	execute_script(rL, L, url);
	return TRUE;
}
...
void initialize() {
	...
	r_lua_pushnumber(rL, (double)(DWORD)HttpGet);
	r_lua_pushcclosure(rL, hook, 1);
	r_lua_setfield(rL, LUA_GLOBALSINDEX, "HttpGet");
...
DWORD WINAPI main() {
	...
	try {
		...
		CreateConsole("Script-Ware");

		int vf_table = addresses::scriptcontext;
		script_context = Scan((char*)&vf_table);
		DWORD dwold;
		if (VirtualProtectEx((HANDLE)-1, (void*)(VMCheckAddy - 3), 1, PAGE_READONLY, &dwold) == 0)
		{
